...
type LangMap = Record<string, unknown>;

const languages: Record<string, LangMap> = { en, fr };
const baseLang = "en";
const baseKeys = Object.keys(languages[baseLang]);

let hasError = false;

console.log(
  `Checking translations against base language: ${baseLang.toUpperCase()} (${baseKeys.length} keys)`,
);
